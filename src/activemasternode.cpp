...
//
// Bootup the masternode, look for a 5000 FRM input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in FerrumCoin.conf */
...
{
    CFerrumCoincoinAddress address(collateralAddress);
