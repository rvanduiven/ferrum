...
void Shutdown()
{
    ...
    RenameThread("FerrumCoin-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: FerrumCoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: FerrumCoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeFerrumCoinamount=<n> " + _("Keep N FerrumCoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. FerrumCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FerrumCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("FerrumCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "FerrumCoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of FerrumCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart FerrumCoin to complete") << "\n";
    ...
    nAnonymizeFerrumCoinAmount = GetArg("-anonymizeferrumcoinamount", 0);
    if(nAnonymizeFerrumCoinAmount > 999999) nAnonymizeFerrumCoinAmount = 999999;
    if(nAnonymizeFerrumCoinAmount < 2) nAnonymizeFerrumCoinAmount = 2;
    ...
    LogPrintf("Anonymize FerrumCoin Amount %d\n", nAnonymizeFerrumCoinAmount);
