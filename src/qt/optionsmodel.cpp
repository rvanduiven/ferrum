...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeFerrumCoinAmount"))
        settings.setValue("nAnonymizeFerrumCoinAmount", 1000);
    nAnonymizeFerrumCoinAmount = settings.value("nAnonymizeFerrumCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeFerrumCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeFerrumCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeFerrumCoinAmount:
            return QVariant(nAnonymizeFerrumCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeFerrumCoinAmount:
            nAnonymizeFerrumCoinAmount = value.toInt();
            settings.setValue("nAnonymizeFerrumCoinAmount", nAnonymizeFerrumCoinAmount);
            emit AnonymizeFerrumCoinAmountChanged(nAnonymizeFerrumCoinAmount);
            break;
        default:
            break;
        }
