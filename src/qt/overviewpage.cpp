...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeFerrumCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeFerrumCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeFerrumCoinAmount = strAnonymizeFerrumCoinAmount.remove(strAnonymizeFerrumCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFerrumCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeFerrumCoinAmount*COIN) nMaxToAnonymize = nAnonymizeFerrumCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeFerrumCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeFerrumCoinAmount));
        strAnonymizeFerrumCoinAmount = strAnonymizeFerrumCoinAmount.remove(strAnonymizeFerrumCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFerrumCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeFerrumCoinAmount == 0){
