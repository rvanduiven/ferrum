...
int nAnonymizeFerrumCoinAmount = 1000;
...
#else
    const char* pszModule = "FerrumCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FerrumCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FerrumCoin
    // Mac: ~/Library/Application Support/FerrumCoin
    // Unix: ~/.FerrumCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FerrumCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "FerrumCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "FerrumCoind.pid"));
